export const CURRENT_FACULTY = [
  {
    name: "İnci Pekgüleç Apaydın",
    isSupervisor: false,
    title: { en: "Instructor", tr: "Öğretim Görevlisi" },
    degree: {
      en: "M.S., Operational Research, Middle East Technical University, 1989",
      tr: "Yüksek lisans, Yöneylem araştırması, Orta Doğu Teknik Üniversitesi, 1989"
    },
    research: [
      { en: "Classical and Exploratory Statistics", tr: "Klasik istatistik" },
      { en: "Forecasting", tr: "kestirim" },
      { en: "Statistical Decision Making", tr: "istatistiksel karar verme kuramı" }
    ],
    office: "SA-144",
    phone: "+90 (312) 290-2131",
    email: "incia [-at-] bilkent.edu.tr",
    photo: "apaydin.jpg",
    webpage: "",
    researchGroups: []
  },

  {
    name: "Fatihcan Atay",
    isSupervisor: true,
    title: { en: "Professor", tr: "Profesör Dr." },
    degree: {
      en: "Ph.D., Applied Mathematics, Brown University, 1994",
      tr: "Doktora, Uygulamalı Matematik, Brown Üniversitesi, 1994"
    },
    research: [
      { en: "Dynamical systems", tr: "Dinamik sistemler" },
      { en: "delay differential equations", tr: "gecikmeli diferansiyel denklemler" },
      { en: "complex systems and networks", tr: "karmaşık sistemler ve ağlar" },
      { en: "applied mathematics", tr: "uygulamalı matematik" }
    ],
    office: "SA-132",
    phone: "+90 (312) 290-1265",
    email: "f.atay [-at-] bilkent.edu.tr",
    photo: "atay-2.jpg",
    webpage: "https://fatihcanatay.wordpress.com/",
    researchGroups: ["analysis", "applied"]
  },

  {
    name: "Cihan Bahran",
    isSupervisor: true,
    title: { en: "Assistant Professor", tr: "Dr. Öğr. Üyesi" },
    degree: {
      en: "Ph.D., Mathematics, University of Minnesota, 2019",
      tr: "Doktora, Matematik, Minnesota Üniversitesi, 2019"
    },
    research: [
      {
        en: "Functorial Representation Theory with applications to Topology",
        tr: "Fonktoryel Temsil Teorisi ve Topoloji Uygulamaları"
      }
    ],
    office: "SA-138",
    phone: "+90 (312) 290-2466",
    email: "cihan.bahran[-at-] bilkent.edu.tr",
    photo: "bahran.jpg",
    webpage: "https://www.cihanbahran.org/",
    researchGroups: ["algebra","geometry"]
  },

  {
    name: "Laurence Barker",
    isSupervisor: true,
    title: { en: "Associate Professor", tr: "Doçent Dr." },
    degree: {
      en: "Ph.D., Mathematics, Oxford University, 1992",
      tr: "Doktora, Matematik, Oxford Üniversitesi, 1992"
    },
    research: [
      { en: "Finite groups", tr: "Sonlu gruplar" },
      { en: "representation theory", tr: "gösterim kuramı" },
      { en: "local and Clifford theory", tr: "Clifford kuramı" },
      { en: "G-algebras", tr: "G-cebirleri" },
      { en: "G-posets", tr: "G-posets" }
    ],
    office: "SA-129",
    phone: "+90 (312) 290-2120",
    email: "barker [-at-] fen.bilkent.edu.tr",
    photo: "barker-2.jpg",
    webpage: "http://www.fen.bilkent.edu.tr/~barker/fenis2.html",
    researchGroups: ["algebra"]
  },

  {
    name: "Alexander Degtyarev",
    isSupervisor: true,
    title: { en: "Professor", tr: "Profesör Dr." },
    degree: {
      en: "Ph.D., Mathematics, Steklov Mathematical Institute, 1988",
      tr: "Doktora, Matematik, Steklov Matematik Enstitüsü, 1988"
    },
    research: [
      { en: "Topology", tr: "Topoloji" },
      { en: "algebraic geometry", tr: "cebirsel geometri" }
    ],
    office: "SA-130",
    phone: "+90 (312) 290-2135",
    email: "degt [-at-] fen.bilkent.edu.tr",
    photo: "degtyarev-2.jpg",
    webpage: "http://www.fen.bilkent.edu.tr/~degt/",
    researchGroups: ["geometry"]
  },

  {
    name: "Alexander Goncharov",
    isSupervisor: true,
    title: { en: "Associate Professor", tr: "Doçent Dr." },
    degree: {
      en: "Ph.D., Mathematics, Rostov State University, 1986",
      tr: "Doktora, Matematik, Rostov Devlet Üniversitesi, 1986"
    },
    research: [
      { en: "Functional analysis", tr: "Fonksiyonel analiz" },
      { en: "operator theory", tr: "" },
      { en: "mathematical physics", tr: "" },
      { en: "numerical analysis", tr: "" }
    ],
    office: "SA-122",
    phone: "+90 (312) 290-1923",
    email: "goncha [-at-] fen.bilkent.edu.tr",
    photo: "goncharov-2.jpg",
    webpage: "http://www.fen.bilkent.edu.tr/~goncha/index.html",
    researchGroups: ["analysis"]
  },

  {
    name: "Ahmet Güloğlu",
    isSupervisor: true,
    title: { en: "Associate Professor", tr: "Doçent Dr." },
    degree: {
      en: "Ph.D., Mathematics, Ohio State University, 2005",
      tr: "Doktora, Matematik, Ohio Eyalet Üniversitesi, 2005"
    },
    research: [{ en: "Analytic number theory", tr: "Analitik sayılar kuramı" }],
    office: "SA-131",
    phone: "+90 (312) 290-2747",
    email: "guloglua [-at-] fen.bilkent.edu.tr",
    photo: "guloglu-2.jpg",
    webpage: "http://www.fen.bilkent.edu.tr/~guloglua/",
    researchGroups: ["arithmetic"]
  },

  {
    name: "Yaghoub Heydarzade",
    isSupervisor: true,
    title: { en: "Assistant Professor", tr: "Dr. Öğr. Üyesi" },
    degree: {
      en: "Ph.D., Physics, Azarbaijan Shahid Madani University, 2018",
      tr: "Doktora, Fizik, Azerbaycan Shahid Madani Üniversitesi, 2018"
    },
    research: [
      { en: "Theoretical physics", tr: "Kuramsal fizik" },
      { en: "gravitation and cosmology", tr: "yerçekimi ve kozmoloji" }
    ],
    office: "SA-137",
    phone: "+90 (312) 290-1590",
    email: "yheydarzade [-at-] bilkent.edu.tr",
    photo: "heydarzade-2.jpg",
    webpage: "https://www.yaghoubheydarzade.com/",
    researchGroups: ["geometry","applied"]
  },

  {
    name: "H. Turgay Kaptanoğlu",
    isSupervisor: true,
    title: { en: "Professor", tr: "Profesör Dr." },
    degree: {
      en: "Ph.D., Mathematics, University of Wisconsin-Madison, 1991",
      tr: "Doktora, Matematik, Wisconsin Üniversitesi, Madison, 1991"
    },
    research: [
      {
        en: "Complex analysis and operator theory in spaces of holomorphic and harmonic functions of several variables",
        tr: "Karmaşık analiz ve operatör kuramı"
      }
    ],
    office: "SA-124",
    phone: "+90 (312) 290-2101",
    email: "kaptan [-at-] fen.bilkent.edu.tr",
    photo: "kaptanoglu-2.jpg",
    webpage: "http://www.fen.bilkent.edu.tr/~kaptan/",
    researchGroups: ["analysis"]
  },

  {
    name: "Enis Kaya",
    isSupervisor: true,
    title: { en: "Assistant Professor", tr: "Dr. Öğr. Üyesi" },
    degree: { en: "Ph. D., University of Groningen", tr: "Doktora, Groningen Üniversitesi" },
    research: [
      { en: "Arithmetic geometry", tr: "Aritmetik geometri" },
      { en: "algebraic geometry", tr: "cebirsel geometri" },
      { en: "tropical geometry", tr: "tropikal geometri" }
    ],
    office: "SA-105",
    phone: "+90 (312) 290 2132",
    email: "enis.kaya [-at-] bilkent.edu.tr",
    photo: "kaya.jpg",
    webpage: "https://sites.google.com/view/eniskaya/home",
    researchGroups: ["arithmetic"]
  },

  {
    name: "Azer Kerimov",
    isSupervisor: true,
    title: { en: "Professor", tr: "Profesör Dr." },
    degree: {
      en: "Ph.D., Mathematics, Landau Institute of Theoretical Physics and Academy of Sciences of Azerbaijan, 1988",
      tr: "Doktora, Matematik, Landau Teorik Fizik Enstitüsü ve Azerbaycan Bilimler Akademisi, 1988"
    },
    research: [
      {
        en: "Mathematical theory of statistical physics",
        tr: "İstatistiksel fiziğin matematiksel kuramı"
      },
      { en: "probability", tr: "olasılık" },
      { en: "mathematical analysis", tr: "matematiksel analiz" }
    ],
    office: "SA-120",
    phone: "+90 (312) 290-1627",
    email: "kerimov [-at-] fen.bilkent.edu.tr",
    photo: "kerimov-2.jpg",
    webpage: "http://www.fen.bilkent.edu.tr/~kerimov/fenis2.html",
    researchGroups: ["analysis","applied"]
  },

  {
    name: "Yasir Kızmaz",
    isSupervisor: true,
    title: { en: "Assistant Professor", tr: "Dr. Öğr. Üyesi" },
    degree: {
      en: "Ph.D., Mathematics, Middle East Technical University, 2018",
      tr: "Doktora, Matematik, Orta Doğu Teknik Üniversites, 2018"
    },
    research: [{ en: "Finite group theory", tr: "Sonlu grup teorisi" }],
    office: "SA-143",
    phone: "+90 (312) 290-2130",
    email: "yasirkizmaz [-at-] bilkent.edu.tr",
    photo: "kizmaz.jpg",
    webpage: "https://myasir-kizmaz.wixsite.com/my-homepage",
    researchGroups: ["algebra"]
  },

  {
    name: "Konstantinos Maronikolakis",
    isSupervisor: true,
    title: { en: "Visiting Assistant Professor", tr: "Misafir Dr. Öğr. Üyesi" },
    degree: {
      en: "Ph.D., Mathematics, University College Dublin, 2024",
      tr: "Doktora, Matematik, University College Dublin, 2024"
    },
    research: [
      { en: "Complex analysis", tr: "Karmaşık analiz" },
      { en: "potential theory", tr: "potansiyel kuramı" },
      { en: "functional analysis", tr: "fonksiyonel analiz" }
    ],
    office: "SA-109",
    phone: "+90 (312) 290-2102",
    email: "konstantinos [-at-] bilkent.edu.tr",
    photo: "maronikolakis.png",
    webpage: "https://conmaro.github.io/",
    researchGroups: ["analysis"]
  },

  {
    name: "Cihan Okay",
    isSupervisor: true,
    title: { en: "Assistant Professor", tr: "Dr. Öğr. Üyesi" },
    degree: {
      en: "Ph.D., Mathematics, University of British Columbia, 2014",
      tr: "Doktora, Matematik, British Columbia Üniversitesi, 2014"
    },
    research: [
      { en: "Algebraic topology", tr: "Cebirsel topoloji" },
      { en: "quantum information theory", tr: "quantum enformasyon kuramı" }
    ],
    office: "SA-140",
    phone: "+90 (312) 290-2103",
    email: "cihan.okay [-at-] bilkent.edu.tr",
    photo: "cihan.jpg",
    webpage: "https://www.cihanokay.com/",
    researchGroups: ["geometry","applied"]
  },

  {
    name: "Türker Özsarı",
    isSupervisor: true,
    title: { en: "Associate Professor, Department Head", tr: "Doçent Dr., Bölüm Başkanı" },
    degree: {
      en: "Ph.D., Mathematics, University of Virginia, 2010",
      tr: "Doktora, Matematik, Virginia Üniversitesi, 2010"
    },
    research: [
      {
        en: "Analysis of partial differential equations and control theory",
        tr: "Kısmi diferansiyel denklemlerin analizi ve kontrol kuramı"
      }
    ],
    office: "SA-111A",
    phone: "+90 (312) 290 2899",
    email: "turker.ozsari [-at-] bilkent.edu.tr",
    photo: "turker.jpg",
    webpage: "https://turkerozsari.com/",
    researchGroups: ["analysis"]
  },

  {
    name: "Naci Saldı",
    isSupervisor: true,
    title: { en: "Associate Professor", tr: "Doçent Dr." },
    degree: {
      en: "Ph.D., Applied Mathematics, Queen's University, 2015",
      tr: "Doktora, Uygulamalı Matematik, Queen's Üniversitesi, 2015"
    },
    research: [
      { en: "Stochastic control theory", tr: "Stokastik denetim kuramı" },
      { en: "game theory", tr: "oyun kuramı" },
      { en: "machine learning", tr: "makine öğrenmesi" },
      { en: "information theory", tr: "enformasyon kuramı" },
      {
        en: "probability theory and its applications",
        tr: "olasılık kuramı ve uygulamaları"
      }
    ],
    office: "SA-139",
    phone: "+90 (312) 290-1938",
    email: "naci.saldi [-at-] bilkent.edu.tr",
    photo: "saldi.jpg",
    webpage: "https://web4.bilkent.edu.tr/saldi/",
    researchGroups: ["analysis","applied"]
  },

  {
    name: "Ali Sinan Sertöz",
    isSupervisor: true,
    title: { en: "Professor", tr: "Profesör Dr." },
    degree: {
      en: "Ph.D., Mathematics, University of British Columbia, 1984",
      tr: "Doktora, Matematik, British Columbia Üniversitesi, 1984"
    },
    research: [{ en: "Algebraic geometry", tr: "Cebirsel geometri" }],
    office: "SA-121",
    phone: "+90 (312) 290-1490",
    email: "sertoz [-at-] bilkent.edu.tr",
    photo: "sertoz-2.jpg",
    webpage: "https://sertoz.bilkent.edu.tr/",
    researchGroups: ["geometry"]
  },

  {
    name: "Müfit Sezer",
    isSupervisor: true,
    title: { en: "Professor", tr: "Profesör Dr." },
    degree: {
      en: "Ph.D., Mathematics, Purdue University, 2003",
      tr: "Doktora, Matematik, Purdue Üniversitesi, 2003"
    },
    research: [
      { en: "Commutative algebra", tr: "Değişmeli cebir" },
      { en: "invariant theory", tr: "değişmezlik teorisi" }
    ],
    office: "SA-117",
    phone: "+90 (312) 290-1255",
    email: "sezer [-at-] fen.bilkent.edu.tr",
    photo: "sezer-2.jpg",
    webpage: "https://orcid.org/0000-0002-6704-5399",
    researchGroups: ["algebra"]
  },

  {
    name: "Syed Waqar Ali Shah",
    isSupervisor: true,
    title: { en: "Assistant Professor", tr: "Dr. Öğr. Üyesi" },
    degree: { en: "Ph.D., Harvard University", tr: "Doktora, Harvard Üniversitesi" },
    research: [
      { en: "Iwasawa Theory", tr: "İwasawa Teorisi" },
      { en: "Arithmetic Geometry", tr: "Aritmetik Geometri" }
    ],
    office: "SA-103",
    phone: "+90 (312) 290-1883",
    email: "swshah [-at-] bilkent.edu.tr",
    photo: "shah.jpg",
    webpage: "https://sites.google.com/view/swshah/home",
    researchGroups: ["arithmetic"]
  },

  {
    name: "Okan Tekman",
    isSupervisor: false,
    title: { en: "Senior Lecturer", tr: "Senior Lecturer" },
    degree: {
      en: "Ph.D., Mathematics, University of Minnesota, 1992",
      tr: "Doktora, Matematik, Minnesota Üniversitesi, 1992"
    },
    research: [{ en: "Automorphic forms", tr: "Otomorfik formlar" }],
    office: "SA-125",
    phone: "+90 (312) 290-1256",
    email: "otekman [-at-] fen.bilkent.edu.tr",
    photo: "tekman-2.jpg",
    webpage: "http://www.fen.bilkent.edu.tr/~otekman/",
    researchGroups: []
  },

  {
    name: "Bülent Ünal",
    isSupervisor: true,
    title: { en: "Associate Professor", tr: "Doçent Dr." },
    degree: {
      en: "Ph.D., Mathematics, University of Missouri, 2000",
      tr: "Doktora, Matematik, Missouri Üniversitesi, 2000"
    },
    research: [
      { en: "Differential geometry", tr: "Diferansiyel geometri" },
      { en: "global analysis on manifolds", tr: "Riemann geometrisi" },
      { en: "general relativity and quantum field theories", tr: "katmanlı uzaylarda global analiz" }
    ],
    office: "SA-118",
    phone: "+90 (312) 290-1719",
    email: "bulent [-at-] fen.bilkent.edu.tr",
    photo: "unal-2.jpg",
    webpage: "http://www.fen.bilkent.edu.tr/~bulent/",
    researchGroups: ["geometry"]
  },

  {
    name: "Özgün Ünlü",
    isSupervisor: true,
    title: { en: "Associate Professor", tr: "Doçent Dr." },
    degree: {
      en: "Ph.D., Mathematics, University of Wisconsin-Madison, 2004",
      tr: "Doktora, Matematik, Wisconsin Üniversitesi, Madison, 2004"
    },
    research: [
     { en: "Algebraic topology", tr: "Cebirsel topoloji" },
     { en: "geometric group theory", tr: "geometrik grup kuramı" },
     { en: "category theory", tr: "kategori kuramı" },
     { en: "group actions", tr: "grup etkileri" }
    ],
    office: "SA-123",
    phone: "+90 (312) 290-1359",
    email: "unluo [-at-] fen.bilkent.edu.tr",
    photo: "unlu-2.jpg",
    webpage: "http://www.fen.bilkent.edu.tr/~unluo/",
    researchGroups: ["geometry", "algebra"]
  },

  {
    name: "Ergün Yalçın",
    isSupervisor: true,
    title: { en: "Professor", tr: "Profesör Dr." },
    degree: {
      en: "Ph.D., Mathematics, University of Wisconsin-Madison, 1998",
      tr: "Doktora, Matematik, Wisconsin Üniversitesi, Madison, 1998"
    },
    research: [
      { en: "Cohomology of groups", tr: "Grup kohomolojisi" },
      {
        en: "finite group actions on topological spaces",
        tr: "topolojik uzaylara sonlu grup etkisi"
      }
    ],
    office: "SA-127",
    phone: "+90 (312) 290-2115",
    email: "yalcine [-at-] fen.bilkent.edu.tr",
    photo: "yalcin-2.jpg",
    webpage: "http://www.fen.bilkent.edu.tr/~yalcine/",
    researchGroups: ["geometry"]
  },

  {
    name: "Hamza Yeşilyurt",
    isSupervisor: true,
    title: { en: "Associate Professor", tr: "Doçent Dr." },
    degree: {
      en: "Ph.D., Mathematics, University of Illinois Urbana-Champaign, 2004",
      tr: "Doktora, Matematik, Illinois Üniversitesi, Urbana-Champaign, 2004"
    },
    research: [{ en: "Analytic number theory", tr: "Analitik sayılar kuramı" }],
    office: "SA-106",
    phone: "+90 (312) 290-1897",
    email: "hamza [-at-] fen.bilkent.edu.tr",
    photo: "yesilyurt-2.jpg",
    webpage: "http://www.fen.bilkent.edu.tr/~hamza/",
    researchGroups: ["arithmetic"]
  },

  {
    name: "Gökhan Yıldırım",
    isSupervisor: true,
    title: { en: "Assistant Professor", tr: "Dr. Öğr. Üyesi" },
    degree: {
      en: "Ph.D., Applied Mathematics, University of Southern California, 2013",
      tr: "Doktora, Uygulamalı Matematik, Güney California Üniversitesi, 2013"
    },
    research: [
      { en: "Enumerative combinatorics", tr: "Saymalı kombinatorik" },
      { en: "probability", tr: "olasılık kuramı" },
      { en: "statistical physics", tr: "istatistiksel fizik" },
      { en: "data science", tr: "veri bilimi" }
    ],
    office: "SA-142",
    phone: "+90 (312) 290-2104",
    email: "gokhan.yildirim [-at-] bilkent.edu.tr",
    photo: "yildirim-2.jpg",
    webpage: "https://orcid.org/0000-0003-4399-7843",
    researchGroups: ["analysis", "applied"]
  },

  {
    name: "Deniz Yılmaz",
    isSupervisor: true,
    title: { en: "Assistant Professor", tr: "Dr. Öğr. Üyesi" },
    degree: {
      en: "Ph.D., Mathematics, University of California, Santa Cruz, 2020",
      tr: "Doktora, Matematik, California Üniversitesi, Santa Cruz, 2020"
    },
    research: [
      { en: "Representation theory", tr: "Temsil kuramı" },
      { en: "finite groups", tr: "sonlu gruplar" },
      { en: "block theory", tr: "blok kuramı" },
      { en: "biset functors", tr: "iküme izleçleri" }
    ],
    office: "SA-114",
    phone: "+90 (312) 290-1085",
    email: "d.yilmaz [-at-] bilkent.edu.tr",
    photo: "d.yilmaz.jpg",
    webpage: "https://yilmazdeniz.bilkent.edu.tr/",
    researchGroups: ["algebra"]
  },

  {
    name: "Natalya Zheltukhina",
    isSupervisor: false,
    title: { en: "Instructor", tr: "Öğretim Görevlisi" },
    degree: {
      en: "Ph.D., Mathematics, Bilkent University, 2002",
      tr: "Doktora, Matematik, Bilkent Üniversitesi, 2002"
    },
    research: [
      { en: "Theory of entire functions", tr: "Tüm düzlemde analitik fonksiyonlar kuramı" },
      { en: "Polya frequency sequences", tr: "Polya sıklık dizileri" }
    ],
    office: "SA-111b",
    phone: "+90 (312) 290-2465",
    email: "natalya [-at-] fen.bilkent.edu.tr",
    photo: "zheltukhina-2.jpg",
    webpage: "http://www.fen.bilkent.edu.tr/~natalya/",
    researchGroups: ["analysis"]
  }
];
